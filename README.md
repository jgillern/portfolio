...
| Provoz | cloud-only single-user aplikace na Vercel Hobby a bezplatných tarifech navazujících služeb; bez lokálního agenta, s auditem importů, monitoringem čerstvosti, kvót a zálohami |
...
Preferovaný a jediný podporovaný přístup první fáze je serverové Gmail API s OAuth a rozsahem `gmail.readonly`. Jednorázový souhlas proběhne v běžném webovém prohlížeči; získaný refresh token se aplikačně zašifruje a uloží do PostgreSQL. Nové zprávy primárně oznamuje Gmail `users.watch` přes Google Cloud Pub/Sub do chráněného webhooku projektu `portfolio-jobs`. Vercel Hobby Cron jednou denně obnoví watch a pomocí Gmail `historyId` dohledá případné zmeškané zprávy. Pokud Pub/Sub nebude aktivní, plně automatický fallback je denní synchronizace s prodlevou až přibližně 24 hodin. IMAP, heslo aplikace ani lokální e-mailový agent nejsou součástí návrhu.
...
- jednorázově se vygeneruje 32bajtový `MASTER_ENCRYPTION_KEY`; jako Vercel Sensitive Environment Variable bude dostupný pouze produkčnímu projektu `portfolio-jobs`;
...
- projekty `portfolio-web`, `portfolio-api`, preview deploymenty a AI nástroje nedostanou master key ani databázové oprávnění k tabulce šifrovaných secretů;
...
Tento model zůstává bezplatný a omezuje dopad úniku samotné databáze, není však ekvivalentem dedikovaného KMS/HSM: kompromitovaný produkční projekt `portfolio-jobs` má během běhu možnost data dešifrovat. Pro osobní Hobby provoz je to vědomý a zdokumentovaný trade-off. Kryptografická vrstva proto musí mít malé vendor-neutral rozhraní, aby ji šlo později nahradit spravovaným secret managerem.
...
Cloud-only architektura běží na Vercel Hobby a nevyžaduje instalaci agenta, lokální plánované úlohy ani úložiště na uživatelově počítači. Veškeré automatické importy, parsování a výpočty spouštějí Vercel Functions; uživatel potřebuje pouze běžný webový prohlížeč.

```mermaid
flowchart TD
    A["George, XTB a Patria e-maily"] --> B["Gmail API"]
    B --> C["Pub/Sub push"]
    D["Webový upload reportu"] --> E["Jobs ingestion"]
    C --> E
    F["Tržní, FX a holdings zdroje"] --> E
    G["Vercel master key"] --> E
    E --> H["Neon a private Blob"]
    H --> I["Dashboard, API a Drive mirror"]
```
...
| --- | --- |
| Hosting a CI/CD | bezplatný Vercel Hobby napojený na GitHub; produkční a preview deployment při změně příslušné větve |
...
| Jobs | Python Vercel Functions v projektu `portfolio-jobs`; parsování, Gmail, ceny, FX, snapshots, zálohy a reconciliace |
| Události a scheduler | Gmail `users.watch` + Google Cloud Pub/Sub pro nové e-maily; Vercel Hobby Cron nejvýše jednou denně pro recovery a dávkové úlohy |
| Databáze | Neon PostgreSQL Free provisionovaný přes Vercel Marketplace; canonical ledger, šifrované dynamické secrety a odvozené snapshoty |
| Raw archive | privátní Vercel Blob na Hobby tarifu; originální dokumenty navíc aplikačně šifrované před uložením |
| Secrets | `MASTER_ENCRYPTION_KEY` jako Vercel Sensitive Environment Variable jen v `portfolio-jobs`; Gmail token a XTB heslo jako AES-256-GCM ciphertext v Neon |
...
Vercel Functions jsou bezstavové a neběží nepřetržitě. Stav úloh drží databázová tabulka jobů. Každý job je idempotentní, používá PostgreSQL advisory lock a umí pokračovat po menších dávkách z checkpointu. Tím se návrh neopírá o nekonečný proces ani o maximální dobu jedné Function invocation.

Redis, Vercel Queues ani Vercel Workflows nejsou pro první osobní verzi povinné. Případné pozdější použití placené nebo beta služby je samostatné rozhodnutí, nikoli skrytá závislost fáze 1.

### 7.2 Nasazení na Vercel Hobby
...
| --- | --- | --- |
| `portfolio-web` | Next.js dashboard a BFF | uživatelská session, omezené read API; bez master key a bez Gmail/XTB secretů |
| `portfolio-api` | FastAPI portfolio API a MCP adapter | PostgreSQL aplikační read roli; bez master key a bez tabulky šifrovaných secretů |
| `portfolio-jobs` | importní a analytické Functions + Cron/webhook endpoints | PostgreSQL write roli, tabulku šifrovaných secretů, private Blob a produkční master key |

Cílová varianta:

- všechny tři projekty zůstávají na Vercel Hobby; fáze 1 nevyžaduje placený Vercel produkt ani automatický upgrade;
...
- preview používá pouze syntetické fixtures a nemá připojení k produkční Neon databázi, Blobu ani produkčním secretům; automatické databázové preview branches se nezakládají;
...
- heslo k XTB PDF je samostatný verzovaný šifrovaný záznam, dostupný pouze importnímu workeru;
...
- raw dokumenty se před uložením aplikačně zašifrují klíčem odvozeným od master key pomocí HKDF s odděleným účelem a uloží do private Vercel Blob s omezenou retencí;
- PostgreSQL poskytne Neon Free přes Vercel Marketplace; Vercel Postgres se nepoužívá, protože pro nové projekty již není dostupný;
...
- lokální vývoj vývojáře může používat pouze syntetická data, ale není součástí uživatelského provozu ani automatizace.

### 7.3 Události, Cron a delší úlohy

- Gmail `users.watch` publikuje změny schránky do Google Cloud Pub/Sub. Push subscription volá produkční HTTPS webhook v `portfolio-jobs`, který ověří Google OIDC token, audience a povolený service account.
- Pub/Sub zpráva neobsahuje portfolio dokument; nese pouze identifikaci změny. Worker si konkrétní zprávu a přílohu načte přes Gmail API s `gmail.readonly`.
- Webhook před zpracováním zapíše idempotentní job. Duplicita nebo retry stejné Pub/Sub notifikace proto nevytvoří duplicitní obchod.
- Vercel Hobby Cron smí každý konkrétní plán spustit nejvýše jednou denně a s hodinovou přesností. Nepoužívá se tedy pro polling po 5–15 minutách.
- Denní recovery job obnoví Gmail watch, projde změny od posledního potvrzeného `historyId` a doplní případné notifikace ztracené během výpadku.
- Další denní Cron endpointy načtou ceny a FX, vytvoří snapshots, provedou rychlou reconciliaci a vytvoří šifrovanou aplikační zálohu. Každý endpoint je autentizovaný, idempotentní a pracuje v UTC.
...
- Pokud Pub/Sub nebude aktivní nebo uživatel nechce připojit Google Cloud billing account, systém zůstane plně automatický přes denní Gmail recovery Cron, jen s prodlevou až přibližně 24 hodin.
...
| --- | --- | --- |
| Vercel Hobby Cron | nejvýše 100 definic na projekt, každá maximálně jednou denně; přesnost v rámci hodiny (±59 minut) | e-mailové události řeší Pub/Sub; Cron je recovery a denní batch |
| Vercel Hobby Functions | 4 hodiny active CPU, 360 GB-h provisioned memory a 1 milion invocations v zahrnutém měsíčním využití; max. 300 s, 2 GB / 1 vCPU na invocation | malé dávky, checkpointy, žádné dlouhé procesy ani častý polling |
| Vercel Blob Hobby | 1 GB uložených dat, prvních 10 000 simple operations, 2 000 advanced operations a 10 GB transferu | privátní šifrovaný archiv s retencí a varováním před vyčerpáním; po překročení může být Blob dočasně nedostupný |
| Neon Free | 100 CU-h/projekt/měsíc, 0,5 GB/projekt, 5 GB public transfer/měsíc, scale-to-zero po 5 minutách, 6hodinové restore okno a jeden ruční snapshot; bez scheduled backups | jeden produkční root branch, kompaktní data a indexy, žádná produkční preview DB; vlastní aplikační záloha |
| Google Cloud Pub/Sub | prvních 10 GiB měsíčního throughputu na billing account zdarma | portfolio notifikace mají zanedbatelný objem; sleduje se usage a aktivují se budget alerty |
...
- rozdělení do tří Vercel projektů se nepovažuje za násobení kvót; spotřeba se vyhodnocuje podle skutečného scope uvedeného poskytovatelem a podle Hobby usage dashboardu;
- dashboard pro import health zobrazuje také Vercel Function usage, Blob storage/operations/transfer, Neon storage/CU-hours/transfer a Pub/Sub throughput;
- varování vzniká při 70 % a 85 % limitu. Při 90 % se odloží look-through refresh, přepočet dlouhé historie a jiné nepovinné joby; importy a záloha mají přednost;
- protože Neon Free nemá plánované zálohy a nabízí jen krátké restore okno, `portfolio-jobs` denně vytvoří komprimovaný aplikační export ledgeru, mapování a konfigurace, zašifruje jej a uloží do private Blobu. Výchozí retence je 7 denních a 4 týdenní body obnovy;
- raw dokumenty a zálohy mají společný kapacitní rozpočet pod 1 GB. Při 70 % se zastaví ukládání duplicitních raw vstupů a UI vyžádá rozhodnutí o zkrácení retence;
- published free-tier limity a reálná měsíční spotřeba se znovu ověří před ostrým spuštěním a poté alespoň čtvrtletně;
...
### 7.5 Navržená struktura monorepa
...
- oddělené databázové role a identity pro importer, web a AI read API;
- pouze produkční `portfolio-jobs` dostane `MASTER_ENCRYPTION_KEY`; web, API, AI a preview prostředí ho nedostanou;
...
- ověření podpisu, audience a service accountu u Pub/Sub push tokenu;
...
Události a plánované úlohy:

| Úloha | Doporučená frekvence |
| --- | --- |
| upozornění na nový Gmail obsah | event-driven přes `users.watch` a Pub/Sub; při nedostupnosti denní fallback |
| obnova Gmail watch a dohledání mezer | jednou denně přes Vercel Hobby Cron |
...
| fund holdings | týdně až měsíčně; denní Cron pouze zjistí, zda je refresh splatný |
...
| kontrola free-tier kvót | denně a před náročným backfillem |

Sledovat minimálně:

- `last_success_at` pro každý konektor a poslední potvrzené Gmail `historyId`;
- expiraci Gmail watch a stáří poslední Pub/Sub notifikace;
...
- Vercel Function CPU/memory/invocations, Blob storage/operations/transfer, Neon storage/CU-hours/transfer a Pub/Sub throughput;
- počet přístupů přes AI API a zamítnuté autorizace.

Alarm má být akční: například „XTB e-mail nebyl 30 dní“ nemusí být chyba, pokud nebyla transakce. Kritičtější je neúspěšné zpracování přijatého výpisu, expirovaný Gmail watch, starý kontrolní snapshot, neobnovitelná záloha nebo rychlé přibližování k free-tier limitu.
...
- pouze `portfolio-jobs` má master key a oprávnění číst `encrypted_secret`; role webu a AI API dostanou explicitní `access denied`;
...
- duplicitní Pub/Sub notifikace nebo opakovaný denní recovery import nevytvoří duplicitní událost;
...
- založit tři Vercel Hobby projekty, Neon Free databázi, private Blob a Google Cloud Pub/Sub push subscription;
- vygenerovat master key, uložit jej pouze do `portfolio-jobs` jako Sensitive Environment Variable a recovery kopii do cloudového password manageru;
- rozhodnout raw/backup retention, single-user autentizaci, použití Pub/Sub billing accountu a EU region;
...
17. reprezentativní měsíc provozu zůstane pod limity Vercel Hobby, Neon Free, Vercel Blob Hobby a Pub/Sub free allowance a žádná komponenta nevyžaduje placený tarif.
...
- zda uživatel aktivuje Google Cloud billing account pro Pub/Sub free allowance; bez něj se použije pouze denní Gmail Cron fallback;
